document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const abonosRes = await fetch(`http://backend-algeciras.hawkins.es/api/abonos/usuario/${usuario.id}`,{
        headers: {
    ...
    for (const partido of partidosFuturos) {
      ...
      const entradaRes = await fetch(`http://backend-algeciras.hawkins.es/api/entradas/buscar-liberada?asientoId=${asientoId}&partidoId=${partido.id}`);
      ...
      if (estaLiberado && !fueComprado) {
        ...
        btn.addEventListener("click", async () => {
          ...
          try {
            const res = await fetch("http://backend-algeciras.hawkins.es/api/abonos/cancelar-liberacion", {
              method: "POST",
        ...
        btn.addEventListener("click", async () => {
          ...
          try {
            const res = await fetch("http://backend-algeciras.hawkins.es/api/abonos/liberar", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                asientoId,
