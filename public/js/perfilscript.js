document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      const res = await fetch(`https://backend-algeciras.hawkins.es:8446/api/abonos/usuario/${usuario.id}`,{
        headers: {
      ...
      const entradasRes = await fetch(`https://backend-algeciras.hawkins.es:8446/api/entradas/usuario/${usuario.id}`,{
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
